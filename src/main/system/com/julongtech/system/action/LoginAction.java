...
public class LoginAction {
			...
			if(StringUtils.isNotEmpty(captcha) && StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(userPassword)){
				...
				if(0 < time  && (System.currentTimeMillis()-time) < 60000){
					if(code.equalsIgnoreCase(captcha)){
						...
						SystemUserDTO systemUserDTO = this.systemUserServiceImpl.getUser(systemUserVO, null);
	...
	public String index(Model model){
		...
		try {
			...
			//根据用户查询菜单
			List<SystemMenuDTO> list = this.authorizeServiceImpl.getUserMenu(systemUserVO, userSession);
			//按钮
			List<SystemButtonDTO> buttonList = this.authorizeServiceImpl.getUserMenuButton(systemUserVO, userSession);
	...
	public String loginOut(){
		...
		UserSession userSession = (UserSession) this.httpSession.getAttribute("userSession");
		if(null != userSession){
			this.httpSession.removeAttribute("userSession");
			this.httpSession.removeAttribute("COUNT");
			this.httpSession.invalidate();
		}
		return "refush";
